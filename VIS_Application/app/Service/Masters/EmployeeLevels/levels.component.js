        ...
        switch (this.dbops) {
            case enum_1.DBOperation.create:
                this._levelsService.post(global_1.Global.BASE_LEVELS_ENDPOINT, formData._value).subscribe(function (data) {
                    if (data == "Success") {
                        _this.msg = "Levels added successfully.";
                        _this.LoadLevelss();
                    }
                    else {
                        _this.msg = "Error has occurred while adding new levels!";
                    }
                    _this.modal.dismiss();
                }, function (error) {
                    _this.msg = error;
            ...
            case enum_1.DBOperation.update:
                this._levelsService.put(global_1.Global.BASE_LEVELS_ENDPOINT, formData._value.Id, formData._value).subscribe(function (data) {
                    if (data == "Success") {
                        _this.msg = "Levels modified successfully.";
                        _this.LoadLevelss();
                    }
                    else {
                        _this.msg = "Error has occurred while modifying existing levels!";
                    }
                    _this.modal.dismiss();
                }, function (error) {
                    _this.msg = error;
