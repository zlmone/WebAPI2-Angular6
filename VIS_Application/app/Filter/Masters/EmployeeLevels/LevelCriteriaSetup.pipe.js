...
var LevelCriteriaSetupFilterPipe = (function () {
    function LevelCriteriaSetupFilterPipe() {
    }
    LevelCriteriaSetupFilterPipe.prototype.transform = function (value, filter) {
        ...
        return filter ? value.filter(function (app) {
            return app.Name != null && app.Name.toLocaleLowerCase().indexOf(filter) != -1
                ...
                || app.CalculatedOn != null && app.CalculatedOn.toLocaleLowerCase().indexOf(filter) != -1;
        }) : value;
    };
    LevelCriteriaSetupFilterPipe = __decorate([
        core_1.Pipe({
            name: 'LevelCriteriaSetupFilter'
        })
    ], LevelCriteriaSetupFilterPipe);
    return LevelCriteriaSetupFilterPipe;
}());
exports.LevelCriteriaSetupFilterPipe = LevelCriteriaSetupFilterPipe;
/*
...
2. In the second line, we are importing the ILevelCriteriaSetup interface that we created in first part to hold the list of currencys.
...
4. Next, we are creating the LevelCriteriaSetupFilterPipe class that is implementing the PipeTransform interface (implementing interface means providing the body to all methods mentioned in the interface).
...
7. So let’s go back to currency.pipe.ts where can see we have transform method with first argument as ILevelCriteriaSetup array and second is named as filter that is the input string to be searched in the ILevelCriteriaSetup array.
